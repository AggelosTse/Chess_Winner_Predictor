...
#helper object to add on the grid_search
...
ConfusionMatrixDisplay.from_estimator(
    best_randomforest, x_test_scaled, y_test, cmap="Blues"
)
...
print(
    f"Standard Deviation (Τυπική Απόκλιση): {grid_search.cv_results_['std_test_score'][best_index_randomForest]:.4f}"
)
...
#helper object to add on the grid_search
param_grid_logisticRegression = {"C": [0.1, 1.0, 10.0], "penalty": ["l1", "l2"]}

grid_logisticRegression = GridSearchCV(
    logistic, param_grid_logisticRegression, cv=3, n_jobs=-1, verbose=1
)
...
ConfusionMatrixDisplay.from_estimator(
    best_logisticRegression, x_test_scaled, y_test, cmap="Blues"
)
...
print(
    f"Standard Deviation (Τυπική Απόκλιση): {grid_logisticRegression.cv_results_['std_test_score'][best_index_logisticRegression]:.4f}"
)
...
#helper object to add on the grid_search
param_grid_supportVectorMachine = {"C": [0.1, 1.0, 10.0], "kernel": ["rbf"]}

grid_supportVectorMachine = GridSearchCV(
    supportVectorMachine, param_grid_supportVectorMachine, cv=3, n_jobs=-1, verbose=1
)
...
#display confusion matrix
ConfusionMatrixDisplay.from_estimator(
    best_supportVectorMachine, x_test_scaled, y_test, cmap="Blues"
)
...
print(
    f"Standard Deviation (Τυπική Απόκλιση): {grid_supportVectorMachine.cv_results_['std_test_score'][best_index_supportVectorMachine]:.4f}"
)
